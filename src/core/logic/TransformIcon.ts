...
export default class TransformIcon extends Laya.Script {
    ...
    @property({ type: Laya.Box, tips: "父节点" })
    public parentNode: Laya.Box;
    ...
    //图片移动回调
    transformMoveHandler: Laya.Handler;
    //图片缩放回调
    transformScaleHandler: Laya.Handler;



    private _scaleStep: number = 0.005;
    private _scalePercent: number = 1;
    private _positionStep: number = 3;
    ...
    resize() {
        this.minScale = this.parentNode.width / this._owner.width;
        this._owner.scale(this.minScale, this.minScale)
        this._maxY = (this.parentNode.height > this._owner.displayHeight) ? (this.parentNode.height - this._owner.displayHeight) / 2 : 0
        this._owner.pos(0, 0)
    }
    ...
    updateLimit(): void {
        //设置初始值
        this._minX = this.parentNode.width - this._owner.displayWidth;
        this._minY = this.parentNode.height - this._owner.displayHeight;
        ...
        if (!this._isHandUp) {
            if (e.touches && e.touches.length > 1) {
                ...
                if (this.setScale(scale)) {
                    ...
                    this.changePositin(new Laya.Vector2(curPiovt.x - this._lastPivot.x, curPiovt.y - this._lastPivot.y))
                }
            } else {
                this.changePositin(new Laya.Vector2(this._owner.mouseX - this._lastPivot.x, this._owner.mouseY - this._lastPivot.y))
            }
        }
    ...
    flexEffect() {
        if (this._owner.scaleX < this.minScale || this._owner.scaleX > this.maxScale) {
            ...
            Laya.Tween.to(this._owner, { scaleX: targetScale, scaleY: targetScale, x: targetPos.x, y: targetPos.y,update: new Laya.Handler(this,()=>{
                this.transformScaleHandler &&  this.transformScaleHandler.runWith(this._owner.scaleX)
            }) }, 200, Laya.Ease.circOut, Laya.Handler.create(this, () => {
                this._isLockPos = false;
                this.updateLimit();
                this.transformScaleHandler &&  this.transformScaleHandler.runWith(null)
            }))
        }else{
            this.transformScaleHandler &&  this.transformScaleHandler.runWith(null) 
        }
        if (this._owner.x < this._minX || this._owner.x > this._maxX || this._owner.y < this._minY || this._owner.y > this._maxY) {
            ...
            Laya.Tween.to(this._owner, { x: targetX, y: targetY, update: new Laya.Handler(this,()=>{
                this.transformMoveHandler &&  this.transformMoveHandler.runWith([this._owner.x,this._owner.y])
            }) }, 200, Laya.Ease.circOut, Laya.Handler.create(this, () => {
                this._isLockPos = false;
                this.transformMoveHandler &&  this.transformMoveHandler.runWith(null)
            }))
        }else{
            this.transformMoveHandler &&  this.transformMoveHandler.runWith(null)
        }
    ...
    private changePositin(direction: Laya.Vector2) {
    ...
    private detectPosition(offsetX: number, offsetY: number) {
        ...
        if (offsetX < (this._minX-this._positionStep) 
        || offsetX > (this._maxX+this._positionStep) 
        || offsetY < (this._minY-this._positionStep*10) 
        || offsetY > (this._maxY+this._positionStep*10)) {
